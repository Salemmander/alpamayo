...
# 4-bit quantized loading (for 10GB VRAM GPUs like RTX 3080)
# No .to("cuda") needed - device_map handles placement
model = AlpamayoR1.from_pretrained("nvidia/Alpamayo-R1-10B", quantization_mode="4bit")

# 8-bit quantized loading (requires ~12GB VRAM):
# model = AlpamayoR1.from_pretrained("nvidia/Alpamayo-R1-10B", quantization_mode="8bit")

# Standard loading (requires 20GB+ VRAM):
